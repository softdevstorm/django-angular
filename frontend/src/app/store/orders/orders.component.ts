...
export class OrdersComponent implements OnInit {
  ...
  searchKeys = ['id', 'customer', 'payment', 'fulfillment', 'status'];
  sortBy = 'id';
  sortDesc = true;
  perPage = 10;
  ...
  view(content, order, options = {}) {
    if (order.products_info) {
      ...
      $('#changedproduct').click();
    }
    
  }

  delete(content, orderId, options = {}) {
    console.log(orderId);
  ...
  changedProduct(content, options) {
    this.modalService.open(content, options).result.then((result) => {
    }, (reason) => {
      // console.log('rejected');
    });
  ...
  submit() {
    ...
    this.api.order.delete(this.deleteId).promise().then(resp => {
      this.orders = this.orders.filter(customer => {
        return customer.id !== this.deleteId;
      });
