...
from api.models import Company, CompanyStuff, Member
...
class UserCreatePasswordView(generics.UpdateAPIView):
    """
    Use this endpoint to create password invited user.
    ...
    def post(self, request, *args, **kwargs):
        ...
        stuff = CompanyStuff.objects.create(company=company, stuff=user)
        stuff.save()
        member = Member(email=user.email, company=store)
        member.save()
        return Response({'success': 'Your account is verified', 'status': 'success'}, status=status.HTTP_200_OK)
        # except Exception as e:
        #     return Response({'success': 'Token expired already', 'status': 'expired'}, status=status.HTTP_200_OK)


class GetUserView(APIView):
    """
    Use this endpoint to get user.
...
class CheckActiveUserView(generics.UpdateAPIView):
    """
    Use this endpoint to create password invited user.
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if user.is_active == True:
                ...
                return Response({'success': 'Please signup', 'status': 'signup'}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({'success': 'Token expired already', 'status': 'expired'}, status=status.HTTP_200_OK)
...
class UploadAvatarView(views.APIView):
    """
    Use this endpoint to upload user avatar.
    """
    @staticmethod
    def post(request):
        try:
